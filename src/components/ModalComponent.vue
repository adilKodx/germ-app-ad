         ...
         <div  class="p-4 " v-if="!success">
            <h1 class="header-2 header mb-4" style="font-size: 20px">
                            ...
                            <div style="float: left; width:80%">
                                <h3 class="mb-2 md:mb-4 header-2-small header"> {{ getLabel('Sla mijn berekening op') }}</h3>
                                </div>
                ...
                <hr/>
                <p class="mb-3 md:mb-6 text-sm text-justify">Om je berekening op te slaan, vul je je e-mailadres in. Wij sturen jou dan een samenvatting van jouw berekening tot nu toe en een link zodat je verder kunt gaan waar je gebleven was.</p>
            <div class="flex flex-col sm:flex-row">
             ...
             <div class="w-full">
                <checkbox 
                      ...
                      :small="true"
                      :checkboxes="[
                         {name:'Ik ga ermee akkoord om vervolgcommunicatie per e-mail te ontvangen over het werk van het Nationaal Zakat Fonds. Ik begrijp ik dat mij elk moment kan afmelden.', value: 1, selected: keepUpToDate}
                     ]">
                </checkbox>
            </div>
             <div class="w-full" style="padding-top: 30px" >
                  <span  v-if="success" class="text-lg" style="color: #1d9868">{{ getLabel('E-mail succesvol verzonden!') }}</span>
             ...
             <div class="w-full">
                ...
                >
                    <span  v-if="!loading" id="send-email-text">{{ getLabel('Sla op') }}</span>
          ...
          <div  class="p-4" v-if="success">
                  <h1 class="header-2 header mb-4" style="font-size: 20px">
                                ...
                                <div style="float: left; width:80%">
                                    <h3 class="mb-2 md:mb-4 header-2-small header text-2xl"> {{ getLabel('Verstuur de berekening') }}</h3>
                                    </div>
                                    <div class="pt-4 w-quater flex justify-end">
                                        <div class="rounded-full border border-red 
                                                    w-3 h-3 md:w-5 md:h-5 flex justify-center
                    ...
                    <hr/>
                    <p class="mb-3 md:mb-6 text-md">Jouw Zakat-berekening is onderweg naar jouw mailbox. Controleer jouw spambox als je de e-mail niet binnen 1 uur hebt ontvangen.</p>
                    <div class="w-full ">
                    <btn id="close-send-calculation-email" 
                    :class="{'-active': true}" 
                    @click="hide"
                    >
                        <span  id="close-send-email-text">{{ getLabel('Sluiten') }}</span>
                    </btn>
                </div>
          </div>
    </modal>
                ...
                if(this.keepUpToDate){
                    ...
                    if(checkError.length <= 0){
                        ...
                        let payload = {
                            ...
                            paramType: 'modlButton',
                        }
                        // localStorage.setItem("calculation", JSON.stringify(cal))
                        this.$store.dispatch('sendCalculationJourny', payload).then(res=> { // adil test
                            ...
                            if( this.keepUpToDate ) {
                                this.keepUpToDate = false
                                document.getElementById("Ik ga ermee akkoord om vervolgcommunicatie per e-mail te ontvangen over het werk van het Nationaal Zakat Fonds. Ik begrijp ik dat mij elk moment kan afmelden.0").click()
                            }
                            this.validates = false
                            this.errorsArr = []
                        }).catch(err=>{
                            ...
                            this.success = false
                            this.error = true
                            this.errorMsg = "Er is iets misgegaan, probeer het later nog eens!"
                        });
                    }
                } else {
                    this.error = true
                    this.errorMsg = "Toestemming vereist"
                    setTimeout(() => {
                        this.error = false
                        this.errorMsg = ""
                    }, 5000);
