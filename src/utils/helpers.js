...
export let numberWithCommas = (num) => {
  num = parseFloat(num);
  num = currencyFormatter.format(num, { locale: "nl-NL" });
  return num; //num.toFixed(2).toString().replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g, ".")
};

export let findOption = (options, alias) => {
  console.log(options);
  console.log(alias);
  var found = options.find((item) => item.alias === alias);
  if (found.active.toUpperCase() === "yes".toUpperCase()) {
    return true;
  } else {
    return false;
  }
...
export let calcPercentOfNum = (num, percent, upOrDown = 1) => {
  if (num) {
    ...
    // let n = new BigNumber(num);
    // n.precision(2, BigNumber.ROUND_UP);
    // let t = n.multipliedBy(new BigNumber(percent).dividedBy(100));
    // let x = parseFloat(t.toFixed(2, upOrDown)).toFixed(2);
    // return Math.round(x * 1e2) / 1e2
    return parseFloat((num / 100) * percent);
  }
  return 0.0;
  //return roundToTwo((num/100) * percent);
};

export let currencyFormat = (num, currency) => {
  let newNum = num > 0 ? roundToTwo(num) : parseFloat(num);

  // if(newNum.split('.')[1] == 0.0){
  //     return '€' + numberWithCommas(newNum.split('.')[0]);
  // }
...
export let submitDonationToApi = (payload) => {
  return new Promise((resolve, reject) => {
    axios
      .post(process.env.VUE_APP_API_URL + "/donation", payload)
      .then((res) => {
        resolve(res);
      })
      .catch((err) => {
...
export let loadPaymentIntent = (payload) => {
  return new Promise((resolve, reject) => {
    axios
      .post(process.env.VUE_APP_API_URL + "/loadPaymentIntent", payload)
      .then((res) => {
        resolve(res);
      })
      .catch((err) => {
...
export let submitDonationToCMS = (payload) => {
  return new Promise((resolve, reject) => {
    axios
      .post(process.env.VUE_APP_API_URL + "/confirm", payload)
      .then((res) => {
        resolve(res);
      })
      .catch((err) => {
