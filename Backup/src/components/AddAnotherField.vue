<template>
  <div
    class="flex w-full flex-col w-full"
        ...
        <div class="flex w-full relative flex-wrap sm:flex-no-wrap">
          <div class="w-full sm:w-1/2 sm:mr-4 mb-6 text-input transition-all">
            <label class="mb-2 sm:hidden flex justify-between">
              ...
              <span>
                <i
                  class="ion-md-trash cursor-pointer text-gray-400"
            ...
            <input
              class="focus:outline-none w-full input-container -large p-4"
              type="text"
          ...
          <div
            class="w-full sm:w-1/2 sm:ml-4 mb-6 relative text-input transition-all"
          >
            <label class="mb-2 sm:hidden text-gray font-semibold ">Waarde</label>

            <div class="relative">
              <input
                class="focus:outline-none w-full input-container -currency -large p-4 pr-4 pl-16"
                :class="{
                  error: field.error,
                }"
                type="number"
          ...
          <div
            @mouseover="showTrash = true"
            class="w-8 h-12 absolute right-0 mt-1 -mr-8"
          >
            <transition name="fade">
              <div
                class="items-center text-gray-400 absolute top-0 w-8 h-full justify-center right-0 hidden sm:flex"
                v-if="showTrash"
              >
                <i
                  class="ion-md-trash cursor-pointer"
                  @click="slideCloseField(field.id)"
                ></i>
              </div>
            </transition>
          </div>
...
export default {
  ...
  methods: {
    ...
    updateStore(field) {
      ...
      switch (this.assetName) {
        ...
        default:
          console.warn(
            'your asset name did not match a switch case to update the store. Check asset name exists, and String format is correct'
          );
      }
    },
    deleteField(id) {
    ...
    amountCheck(field) {
      if (field.value.length > 0) {
        this.fields.filter((el) => el.id == field.id)[0].error = !amountCheck(
          field.value
        );
      }
    },
  ...
  mounted() {
    ...
    this.activeKeys =
      this.fields.map((el) => el.id).length > 0
        ? this.fields.map((el) => el.id)
        : [];
