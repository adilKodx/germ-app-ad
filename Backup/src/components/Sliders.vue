<template>
  <div class="w-full">
    ...
    <h3 class="header header-2 mb-4">{{ getLabel("How you can help") }}</h3>
    <!-- Add sm:text-lg -->
    <p class="mb-4">
...
export default {
  ...
  methods: {
    ...
    isDragging: _.throttle(function () {
      if (this.allocation !== "custom") {
        this.$store.commit("SET_ALLOCATION", "custom");
  ...
  computed: {
    buttons() {
      return [
        ...
        {
          name: "Gelijk verdelen",
          value: "2",
          selected: this.allocation == "even" ? true : false,
        },
        {
          name: "Ik wil beslissen",
          value: "3",
  ...
  mounted() {
    // console.log(this.donationSplit)
